<template>
  <div class="mymapdata" id="customPopUpDiv">
    <div class="label1">
      ...
      <div class="context_div" v-show="realtime_show">
        ...
        <div class="popu_chart_btn_div">
          <div
            class="popu_chart_btn popu_chart_btn_active class1"
      ...
      <div style="width:330px;height:30px;margin:10px auto;">
        <div class="bottom_btn" @click="from_show('0',$event)">发起排查</div>
        <div class="bottom_btn" @click="from_show('1',$event)">发起管控</div>
        <div class="bottom_btn" @click="close_warm()">关闭警报</div>
      </div>
    </div>
    ...
    <div class="popup_from_div" v-show="paicha_show">
    ...
    <div class="popup_from_div" style="height:380px;" v-show="guankong_show">
      <div style="float:left;width:100%;height:40px;">
...
export default {
  ...
  computed: {
    myPointLayer() {
      return this.$store.state.PollutionSource.map;
    }
  },
  watch: {
  ...
  methods: {
    ...
    popup_init: function() {
      ...
      let cVue = this;
      if (cVue.$store.state.PollutionSource.mapview != null) {
        cVue.$store.state.PollutionSource.mapview.on("click", function(event) {
          cVue.$store.state.PollutionSource.mapview.hitTest(event).then(function(response) {
            if (response.results.length) {
              ...
              // }
              //监听地图缩放
              cVue.$store.state.PollutionSource.mapview.watch("extent", newValue => {
    ...
    map_click(stationid, stationtype) {
      ...
      //地图以弹出点居中高亮显示
      this.$store.state.PollutionSource.mapview.center = [this.longitude, this.latitude];
      ...
      if (
        ...
        layer = cVue.$store.state.PollutionSource.map.findLayerById("airPointFeaLayer");
      } else if (stationtype == stationtypeobj.wanranyuan) {
        layer = cVue.$store.state.PollutionSource.map.findLayerById("wryPointFeaLayer");
      } else if (stationtype == stationtypeobj.yangchen) {
        layer = cVue.$store.state.PollutionSource.map.findLayerById("ycPointFeaLayer");
      }
      if (layer != null) {
        this.$store.state.PollutionSource.mapview
          .whenLayerView(layer)
          .then(layerView => {
          ...
          if (response.status == 200) {
            if (response.data == null || response.data == "") {
              ...
              if (stationtype == stationtypeobj.wanranyuan) {
                ...
                $("#popu_line_chart").css("height", "100px");
                cVue.chart2_show = false;
                cVue.chart1_show = true;
              }
              let mapChart = echarts.init(
    ...
    resetPopUpPositon: function() {
      ...
      let screenPoint = this.$store.state.PollutionSource.mapview.toScreen(mapPoint);
