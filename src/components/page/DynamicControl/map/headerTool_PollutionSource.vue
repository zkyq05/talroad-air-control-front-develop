...
export default {
  name: "HeaderToolButtonpollution",
  data() {
    return {
      fullscreen: false,
  ...
  methods: {
    ...
    zoominBtn: function(event) {
      let curMapView = this.$store.state.PollutionSource.mapview;
          ...
          ([Basemap, WebTileLayer, TileLayer]) => {
            ...
            if (kind == "01") {
              let baseMap = new Basemap({
                baseLayers: [myBaseTileLayer], //影像底图和注记层
                title: "蓝黑底图",
                id: "myBlackBlueBasemap"
              });
              cVue.$store.state.PollutionSource.map.basemap = baseMap;
            }
            else if (kind == "02") {
              let baseMap = new Basemap({
    ...
    measurement_line: function(e) {
      ...
      if(color=="rgb(0, 255, 193)"){
        mydom.html("移除");
        mydom.addClass("is-active");
        let Map = this.$store.state.PollutionSource.map;
        let curMapView = this.$store.state.PollutionSource.mapview;
    ...
    addRoadData:function(e){
      ...
      loadModules([
        ...
        let layer = this.$store.state.PollutionSource.map.findLayerById("gdRoadLayer");
        if(layer!=null){
          ...
          this.$store.state.PollutionSource.map.add(gdroad);
        }
    ...
    //全图
    setFull:function(){
      this.$store.state.PollutionSource.mapview.extent = mapExtent;
    },
    //数值、名称渲染
    setRenderType:function(selectedType,e){
      ...
      if(color=="rgb(0, 255, 193)"){
        mydom.addClass("is-active");
        this.$store.state.PollutionSource.curRenderType=1
      }else{
        mydom.removeClass("is-active");
        this.$store.state.PollutionSource.curRenderType=0
      }
